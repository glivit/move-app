        ...
        <head>
          ...
          <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .info-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #F0F0ED; font-size: 12px; }
            ...
            .compliance-track { flex: 1; height: 8px; background: #F0F0ED; border-radius: 4px; overflow: hidden; }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-[#8B6914]" strokeWidth={1.5} />
      </div>
    )
  }
  ...
  if (!report) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] p-6">
        <Link href={`/coach/clients/${params.id}`} className="inline-flex items-center gap-2 text-[#8B6914] mb-6">
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link href={`/coach/clients/${params.id}`} className="inline-flex items-center gap-2 text-[#8B6914] font-medium text-[15px]">
          <ArrowLeft className="w-5 h-5" strokeWidth={1.5} /> Terug
        </Link>
        <button
          onClick={handleDownloadPDF}
          disabled={generating}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#8B6914] text-white rounded-xl font-semibold text-[14px] hover:bg-[#6B5110] transition-colors disabled:opacity-50"
        >
          {generating ? (
      ...
      {/* Preview card */}
      <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06)] border border-[#F0F0ED] overflow-hidden">
        <div className="p-4 bg-[#8B6914]/5 border-b border-[#F0F0ED] flex items-center gap-3">
          <FileText className="w-5 h-5 text-[#8B6914]" strokeWidth={1.5} />
          <div>
        ...
        <div ref={reportRef} className="p-6">
          {/* Header */}
          <div className="report-header" style={{ textAlign: 'center', paddingBottom: 20, borderBottom: '2px solid #8B6914', marginBottom: 24 }}>
            <h1 style={{ fontSize: 28, color: '#8B6914', fontWeight: 700, letterSpacing: 2 }}>MŌVE</h1>
          ...
          <div className="section" style={{ marginBottom: 24 }}>
            <div className="section-title" style={{ fontSize: 16, fontWeight: 700, marginBottom: 12, paddingBottom: 6, borderBottom: '1px solid #F0F0ED' }}>
              Cliënt Overzicht
            </div>
            <div style={{ display: 'grid', gap: 4 }}>
              {[
                ...
                <div key={row.label} className="info-row" style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #F0F0ED', fontSize: 12 }}>
                ...
                {Object.entries(report.training.volumeByGroup)
                  ...
                  .map(([group, volume]) => (
                    <div key={group} className="bar-container" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1, height: 16, background: '#F0F0ED', borderRadius: 4, overflow: 'hidden' }}>
                        <div className="bar-fill" style={{ width: `${(volume / maxGroupVolume) * 100}%`, height: '100%', borderRadius: 4, background: '#8B6914' }} />
                      </div>
              ...
              <table className="pr-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                ...
                <tbody>
                  {report.training.personalRecords.map((pr, i) => (
                    <tr key={i}>
                      <td style={{ padding: '6px 8px', borderBottom: '1px solid #F0F0ED' }}>{pr.exercise}</td>
                      <td style={{ padding: '6px 8px', borderBottom: '1px solid #F0F0ED' }}>
                        {pr.type === 'weight' ? 'Max gewicht' : pr.type === 'reps' ? 'Max reps' : 'Volume'}
                      </td>
                      <td style={{ padding: '6px 8px', borderBottom: '1px solid #F0F0ED', textAlign: 'right', fontWeight: 700, color: '#AF52DE' }}>
                        {pr.value} {pr.type === 'weight' ? 'kg' : pr.type === 'reps' ? 'reps' : 'kg'}
                      </td>
                      <td style={{ padding: '6px 8px', borderBottom: '1px solid #F0F0ED', textAlign: 'right', color: '#8E8E93' }}>
          ...
          <div className="section" style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              <div className="compliance-bar" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div className="compliance-track" style={{ flex: 1, height: 8, background: '#F0F0ED', borderRadius: 4, overflow: 'hidden' }}>
          ...
          {/* Footer */}
          <div className="footer" style={{ marginTop: 32, paddingTop: 12, borderTop: '1px solid #F0F0ED', textAlign: 'center', fontSize: 10, color: '#C7C7CC' }}>
