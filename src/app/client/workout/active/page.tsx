...
// Heavy modals — lazy-loaded via next/dynamic. Niet nodig in initial
// page bundle: pas downloaden + parsen wanneer user op de relevante
// CTA tikt. Saves ~500-800 LOC + recharts/extra deps from initial load.
const ExercisePickerModal = dynamic(
  () => import('@/components/workout/ExercisePickerModal').then(m => m.ExercisePickerModal),
  { ssr: false },
)
const FormCheckModal = dynamic(
  () => import('@/components/workout/FormCheckModal').then(m => m.FormCheckModal),
  { ssr: false },
)
import {
  X,
